...
func TestRemote(t *testing.T) {
	...
	server.SetREClient(env.Client, func() {})
	lg, err := logger.New(logger.TextFormat, env.ExecRoot, stats.New(), nil, nil, nil, nil)
	if err != nil {
	...
	env.Set(
		wantCmd, command.DefaultExecutionOptions(), res,
		fakes.StdErr(wantStdErr),
		&fakes.OutputFile{Path: abOutPath, Contents: "output", IsExecutable: true},
	)
	for i := 0; i < 2; i++ {
...
func TestRemote_CanonicalWorkingDir(t *testing.T) {
	...
	env.Set(
		wantCmd,
		command.DefaultExecutionOptions(), res,
		fakes.StdErr(wantStdErr),
		&fakes.OutputFile{Path: abOutPath, Contents: "output", IsExecutable: false},
	)
...
func TestRemoteWithPreserveUnchangedOutputMtime(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			env.Set(
				wantCmd, command.DefaultExecutionOptions(), res,
				&fakes.OutputFile{Path: tc.testFile, Contents: tc.wantOutput, IsExecutable: false},
			)
...
func TestRemoteWithSingleActionLog(t *testing.T) {
	...
	res := &command.Result{Status: command.CacheHitResultStatus}
	wantStdErr := []byte("stderr")
	env.Set(wantCmd, command.DefaultExecutionOptions(), res, fakes.StdErr(wantStdErr),
		&fakes.OutputFile{Path: abOutPath, Contents: "output", IsExecutable: false},
	)
...
func TestLERCNonShallowValidCacheHit(t *testing.T) {
	...
	env.Set(
		wantCmd, command.DefaultExecutionOptions(), res,
		&fakes.OutputFile{Path: "foo.d", Contents: dFileContent, IsExecutable: false},
		&fakes.OutputFile{Path: "foo.o", Contents: "foo.o", IsExecutable: false},
	)
	got, err := server.RunCommand(ctx, req)
	if err != nil {
		t.Errorf("RunCommand() returned error: %v", err)
...
func TestLERCDepsValidCacheHit(t *testing.T) {
	...
	env.Set(wantCmd, command.DefaultExecutionOptions(), res,
		&fakes.OutputFile{Path: "foo.d", Contents: dFileContent, IsExecutable: false},
		&fakes.OutputFile{Path: "foo.d.deps", Contents: depsFileContent, IsExecutable: false},
		&fakes.OutputFile{Path: "foo.o", Contents: "foo.o", IsExecutable: false},
	)
	got, err := server.RunCommand(ctx, req)
	if err != nil {
...
func TestLERCMismatches(t *testing.T) {
	...
	env.Set(wantCmd, executionOptions, res,
		&fakes.OutputFile{Path: "foo.d", Contents: dFileContent, IsExecutable: false},
		&fakes.OutputFile{Path: "foo.d.deps", Contents: depsFileContent, IsExecutable: false},
		&fakes.OutputFile{Path: "foo.o", Contents: "surprize!", IsExecutable: false},
	)
...
func TestCompareStashRestore(t *testing.T) {
	...
	executionOptions.DoNotCache = true
	env.Set(wantCmd, executionOptions, res,
		&fakes.OutputFile{Path: "inout", Contents: "bar", IsExecutable: false},
	)
	got, err := server.RunCommand(ctx, req)
	if err != nil {
		t.Errorf("RunCommand() returned error: %v", err)
...
func TestNumRetriesIfMismatched(t *testing.T) {
	...
	executionOptions := command.DefaultExecutionOptions()
	executionOptions.DoNotCache = true
	env.Set(cmd, executionOptions, res,
		&fakes.OutputFile{Path: abOutPath, Contents: "fake-output", IsExecutable: false},
		&fakes.OutputDir{Path: abPath},
	)
...
func TestCompareWithRerunsNoMismatches(t *testing.T) {
	...
	executionOptions.DoNotCache = true
	env.Set(cmd, executionOptions, res,
		&fakes.OutputFile{Path: abOutPath, Contents: string(wantOutput), IsExecutable: false}, &fakes.OutputDir{Path: abPath})
	got, err := server.RunCommand(ctx, req)
	if err != nil {
		t.Errorf("RunCommand() returned error: %v", err)
	}
...
func TestCompareWithReruns(t *testing.T) {
	...
	executionOptions.DoNotCache = true
	env.Set(cmd, executionOptions, res,
		&fakes.OutputFile{Path: abOutPath, Contents: "fake-output", IsExecutable: false}, &fakes.OutputDir{Path: abPath},
	)
	os.RemoveAll(filepath.Join(env.ExecRoot, abPath))
	got, err := server.RunCommand(ctx, req)
	if err != nil {
...
func TestRacingRemoteWinsCopyWorksOnTmpFs(t *testing.T) {
	...
	res := &command.Result{Status: command.SuccessResultStatus}
	env.Set(cmd, command.DefaultExecutionOptions(), res,
		&fakes.OutputFile{Path: abOutPath, Contents: string(wantOutput), IsExecutable: false}, fakes.StdOut("Done"),
	)
...
func TestRacingRemoteFailsLocalWins(t *testing.T) {
	...
	env.Set(cmd, command.DefaultExecutionOptions(), res,
		&fakes.OutputFile{Path: abOutPath, Contents: string(wantOutput), IsExecutable: false}, fakes.StdOut(wantStdout),
	)
	got, err := server.RunCommand(ctx, req)
	if err != nil {
		t.Errorf("RunCommand() returned error: %v", err)
...
func TestRacing_DownloadOutputs(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := &command.Result{Status: command.SuccessResultStatus}
			env.Set(wantCmd, command.DefaultExecutionOptions(), res,
				&fakes.OutputFile{Path: abPath, Contents: wantOutput, IsExecutable: false},
			)
...
func TestRacingRemoteWins_PreserveUnchangedOutputMtime(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			res := &command.Result{Status: command.SuccessResultStatus}
			env.Set(wantCmd, command.DefaultExecutionOptions(), res,
				&fakes.OutputFile{Path: tc.testFile, Contents: wantOutput, IsExecutable: false},
			)
			time.Sleep(time.Second) // Wait for mtime to catch up
			oldDigest, oldMtime := getFileInfo(t, path)
			if _, err := server.RunCommand(ctx, req); err != nil {
...
func TestRacingLocalWinsIfStarted(t *testing.T) {
	...
	res := &command.Result{Status: command.SuccessResultStatus}
	env.Set(cmd, command.DefaultExecutionOptions(), res,
		&fakes.OutputFile{Path: abOutPath, Contents: string(wantOutput), IsExecutable: false},
	)
...
func TestRemoteRacingFinishInReasonableTimeDuration(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			env.Set(cmd, command.DefaultExecutionOptions(), res,
				&fakes.OutputFile{Path: abOutPath, Contents: "", IsExecutable: false},
			)
			startTime := time.Now()
...
func TestRemoteDisabled(t *testing.T) {
	...
	setPlatformOSFamily(wantCmd)
	res := &command.Result{Status: command.CacheHitResultStatus}
	env.Set(wantCmd, command.DefaultExecutionOptions(), res, fakes.StdErr("remoteStderr"),
		&fakes.OutputFile{Path: abOutPath, Contents: "output", IsExecutable: false},
	)
